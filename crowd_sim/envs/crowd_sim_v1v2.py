...
class CrowdSim(gym.Env):
    ...
    def step(self, action, update=True):
        ...
        """
        # 让每个人类选择动作，对每个 human构造观测 ob，调用 human.act(ob) 得到 human_actions
        human_actions = []
        for human in self.humans:
        ...
        # collision detection 碰撞检测 & 终止条件 机器人和每个 human 之间 检测碰撞
        dmin = float('inf')
        collision = False
        for i, human in enumerate(self.humans):
        ...
        # collision detection between humans 人与人之间检测碰撞
        ...
        if update:
            ...
            self.states.append([self.robot.get_full_state(), [human.get_full_state() for human in self.humans]])
            # 有些机器人策略（例如 value-based）会在决策时计算每个离散动作的价值 action_values，这里把它存起来用于渲染里的 value heatmap。
            if hasattr(self.robot.policy, 'action_values'):
                self.action_values.append(self.robot.policy.action_values)
            # 如果机器人策略有注意力机制，这里存下来用于视频模式显示“Human i: attention”。
            if hasattr(self.robot.policy, 'get_attention_weights'):
                self.attention_weights.append(self.robot.policy.get_attention_weights())

            # update all agents
            self.robot.step(action)
            # 对每个 human，把前面算出来的 human_action 应用到这个人的状态上。
            ...
            # 计算并返回给机器人的下一时刻观测 ob
            # 机器人观测是“所有 humans 的 observable state 列表”（只包含可观测部分，如位置速度半径等，不含其目标）。
            if self.robot.sensor == 'coordinates':
                ob = [human.get_observable_state() for human in self.humans]
            elif self.robot.sensor == 'RGB':
                raise NotImplementedError
        else: # else: 分支：不更新环境，只做一步“前瞻”推断（planning 用）
            if self.robot.sensor == 'coordinates':
